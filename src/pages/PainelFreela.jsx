...
export default function PainelFreela() {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged((user) => {
      ...
      if (!user || !localUser || localUser.tipo !== 'freela') {
  ...
  const fazerCheckout = async () => {
    ...
    const chamadaPendenteCheckout = chamadas.find(
      (c) => c.checkInFreela && !c.checkOutFreela && c.status === 'aceita'
    )
          ...
          <div className="bg-white rounded-2xl shadow p-6">
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <div className="mt-4">
                  <p><strong>Último Check-in:</strong> {formatTimestamp(freela.checkIn)}</p>
                  <p><strong>Último Check-out:</strong> {formatTimestamp(freela.checkOut)}</p>
                </div>
                <div className="flex gap-4 mt-3">
                  <button onClick={fazerCheckin} disabled={loadingCheckin} className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition">
                    {loadingCheckin ? 'Registrando...' : 'Fazer Check-in'}
                  </button>
                  <button onClick={fazerCheckout} disabled={loadingCheckout} className="px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 transition">
          ...
          <div className="bg-white rounded-2xl shadow p-6 max-h-[500px] overflow-auto">
            ...
            {chamadas.map((chamada) => (
              <div key={chamada.id} className="mb-4 p-3 border rounded">
                ...
                <p><strong>Check-in feito por você:</strong> {chamada.checkInFreela ? 'Sim' : 'Não'}</p>
                <p><strong>Check-in confirmado:</strong> {chamada.checkInConfirmadoHora ? formatTimestamp(chamada.checkInConfirmadoHora) : 'Não'}</p>
                <p><strong>Check-out feito:</strong> {chamada.checkOutFreela ? 'Sim' : 'Não'}</p>
                <p><strong>Check-out confirmado:</strong> {chamada.checkOutConfirmado ? 'Sim' : 'Não'}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-10">
          ...
          {vagas.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {vagas.map((vaga) => (
                <div key={vaga.id} className="bg-white p-4 rounded-lg shadow hover:shadow-lg transition cursor-pointer" onClick={() => navigate(`/vaga/${vaga.id}`)}>
                  <h3 className="text-lg font-bold text-gray-800">{vaga.titulo || 'Título não informado'}</h3>
                  <p><strong>🏢</strong> {vaga.empresa || 'Empresa não informada'}</p>
                  <p><strong>📍</strong> {vaga.cidade || 'Cidade não informada'}</p>
                  <p><strong>💰</strong> {vaga.valorDiaria ? `R$ ${vaga.valorDiaria.toFixed(2)}` : vaga.salario || '—'}</p>
                  <p className="text-sm text-gray-600 mt-1">{vaga.descricao || 'Sem descrição'}</p>
                </div>
              ))}
            </div>
          )}
