...
export default function PainelFreela() {
  ...
  const [audioChamada] = useState(() => new Audio('https://res.cloudinary.com/dbemvuau3/video/upload/v1750961914/qhkd3ojkqhi2imr9lup8.mp3'));
  ...
  const carregarFreela = useCallback(async () => {
    ...
    try {
      ...
      const q = query(chamadasRef, where('freelaUid', '==', usuario.uid), orderBy('criadoEm', 'desc'));
  ...
  useEffect(() => {
    const carregarDados = async () => {
      ...
      const vagasLocal = localStorage.getItem('vagas');
      const vagasDisponiveis = vagasLocal ? JSON.parse(vagasLocal) : [];
