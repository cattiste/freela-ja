...
export default function PainelPessoaFisica() {
  ...
  useEffect(() => {
    const rota = location.pathname.split('/pf/')[1];
    if (rota) setAbaSelecionada(rota);
  }, [location]);
  ...
  const renderPerfil = () => (
    <div>
      ...
      <div className="mt-4 text-center">
        <a href="/cadastro-evento" className="bg-orange-500 text-white px-4 py-2 rounded-full shadow hover:bg-orange-600">
          📆 Publicar Evento
        </a>
      </div>
    </div>
  ...
  const renderConteudo = () => {
    switch (abaSelecionada) {
      case 'buscar':
        return <BuscarFreelas tipoChamador={pessoa_fisica} usuariosOnline={usuariosOnline} />
      case 'candidaturas':
  ...
  return (
    <div className="min-h-screen bg-cover bg-center p-4 pb-20" style={{
      backgroundImage: `url('/img/fundo-login.jpg')`,
      backgroundAttachment: 'fixed',
      backgroundRepeat: 'no-repeat',
      backgroundSize: 'cover',
    }}>
      <div className="pb-24">{renderConteudo()}</div>
      <MenuInferiorPF />
    </div>
  );
}
