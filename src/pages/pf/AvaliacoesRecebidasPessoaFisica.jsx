// src/pages/pf/AvaliacoesRecebidasPessoaFisica.jsx
import React, { useEffect, useState } from 'react'
import { collection, query, where, getDocs } from 'firebase/firestore'
import { db } from '@/firebase'
import { useAuth } from '@/context/AuthContext'

export default function AvaliacoesRecebidasPessoaFisica() {
    ...
    const buscarAvaliacoes = async () => {
      ...
      try {
        ...
        const avaliadas = snap.docs
          ...
          .filter(chamada => chamada.avaliacaoPessoaFisica?.nota)
      ...
      {avaliacoes.map((chamada) => (
        <div key={chamada.id} className="bg-white border p-4 rounded-xl shadow-sm">
          ...
          <p className="text-gray-800 italic mt-1">
            "{chamada.avaliacaoPessoaFisica?.comentario || 'Sem comentário'}"
          </p>
          <p className="text-yellow-600 mt-2">⭐ Nota: {chamada.avaliacaoPessoaFisica?.nota || '--'}</p>
        </div>
      ))}
    </div>
  )
